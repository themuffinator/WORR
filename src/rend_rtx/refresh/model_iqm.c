...
static inline void QuatCopy(const quat_t in, quat_t out)
{
	out[0] = in[0];
	out[1] = in[1];
	out[2] = in[2];
...
static inline void AddPointToBoundsLocal(const vec3_t v, vec3_t mins, vec3_t maxs)
{
	for (int i = 0; i < 3; i++) {
		vec_t val = v[i];
...
int MOD_LoadIQM_Base(model_t* model, const void* rawdata, size_t length, const char* mod_name)
{
	...
	else if (header->num_meshes && header->num_frames == 0)
	{
		...
		{
			AddPointToBoundsLocal(&iqmData->positions[vertex_idx * 3], &iqmData->bounds[0], &iqmData->bounds[3]);
		}
	}
