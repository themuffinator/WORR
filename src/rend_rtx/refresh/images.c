    ...
    char        path[MAX_OSPATH];
    {
        ...
        for (; argi < argc; argi++) {
            ...
            if (!strncmp(arg, "--save=", 7)) {
                ...
                if (argi + 1 >= argc) {
                    Com_WPrintf("missing save path for --save\n");
                    return;
                }
                optarg = Cmd_Argv(++argi);
            } else if (!strcmp(arg, "-S")) {
                if (argi + 1 >= argc) {
                    Com_WPrintf("missing save path for -S\n");
                    return;
                }
                optarg = Cmd_Argv(++argi);
            }
            ...
            if (!strcmp(arg, "-p") || !strcmp(arg, "--pics"))
                mask |= 1 << IT_PIC;
            else if (!strcmp(arg, "-f") || !strcmp(arg, "--fonts"))
                mask |= 1 << IT_FONT;
            else if (!strcmp(arg, "-m") || !strcmp(arg, "--skins"))
                mask |= 1 << IT_SKIN;
            else if (!strcmp(arg, "-s") || !strcmp(arg, "--sprites"))
                mask |= 1 << IT_SPRITE;
            else if (!strcmp(arg, "-w") || !strcmp(arg, "--walls"))
                mask |= 1 << IT_WALL;
            else if (!strcmp(arg, "-y") || !strcmp(arg, "--skies"))
                mask |= 1 << IT_SKY;
            else if (!strcmp(arg, "-P") || !strcmp(arg, "--placeholder"))
                placeholder = true;
...
void IMG_GetPalette(void)
{
    ...
    // 255 is transparent
    d_8to24table[i] = COLOR_U32_RGBA(src[0], src[1], src[2], 0);
    return;
