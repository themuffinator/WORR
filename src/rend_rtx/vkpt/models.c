...
static void extract_model_lights(model_t* model)
{
	...
	if (!(model->light_polys = Hunk_TryAlloc(&model->hunk, sizeof(model->light_polys[0]) * num_lights, MOD_MALLOC_ALIGN))) {
...
int MOD_LoadIQM_RTX(model_t* model, const void* rawdata, size_t length, const char* mod_name)
{
	...
	COM_SplitPath(mod_name, model_name, sizeof(model_name), base_path, sizeof(base_path), true);
	(void)model_name;
	size_t base_len = strlen(base_path);
	if (base_len > 0 && (base_path[base_len - 1] == '/' || base_path[base_len - 1] == '\\'))
