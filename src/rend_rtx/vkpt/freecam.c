...
#undef Key_IsDown
#endif
#ifdef Key_GetBindingForKey
#undef Key_GetBindingForKey
#endif
...
static void vkpt_freecam_update_cvars(void)
{
	if (!sensitivity)
	...
	if (!m_invert)
		m_invert = Cvar_Get("m_invert", "0", 0);
}

static void vkpt_freecam_get_autosens(float *out_x, float *out_y)
{
	float fov;

	if (m_autosens && m_autosens->value > 90.0f && m_autosens->value <= 179.0f)
		fov = m_autosens->value;
	else
		fov = 90.0f;
...
static void vkpt_freecam_mousemove(void)
{
	...
	else if (Key_IsDown(K_MOUSE1))
	{
		...
		if (m_autosens->integer && vkpt_refdef.fd) {
			float autosens_x;
			float autosens_y;

			vkpt_freecam_get_autosens(&autosens_x, &autosens_y);
			mx *= vkpt_refdef.fd->fov_x * autosens_x;
			my *= vkpt_refdef.fd->fov_y * autosens_y;
		}
...
void vkpt_freecam_update(float frame_time)
{
	...
	if (!freecam_active)
	{
		...
		freecam_zoom = 1.f;
		freecam_player_model = cl_player_model ? cl_player_model->integer : CL_PLAYER_MODEL_DISABLED;
		freecam_active = true;
	}
	...
	{
		if (cl_player_model && freecam_player_model != CL_PLAYER_MODEL_DISABLED && cl_player_model->integer != CL_PLAYER_MODEL_THIRD_PERSON)
