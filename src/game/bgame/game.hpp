...
#ifdef GAME_INCLUDE
using gvec3_t = Vector3;
using gvec3_ptr_t = Vector3 *;
using gvec3_cptr_t = const Vector3 *;
using gvec3_ref_t = Vector3 &;
using gvec3_cref_t = const Vector3 &;
...
struct cvar_t {
	...
	cvar_flags_t	flags;
	int32_t			modifiedCount; // changed each time the cvar is changed, but never zero
	float			value;
	cvar_t			*next;
...
inline bool Cvar_WasModified(const cvar_t *cvar, uint32_t &modified) {
	if (cvar->modifiedCount != modified) {
		modified = cvar->modifiedCount;
		return true;
	}
...
struct csurface_t {
	char			name[32];
	surfflags_t		flags = SURF_NONE;
	int32_t			value = 0;

	// [Paril-KEX]
	uint32_t		id = 0; // unique texinfo ID, offset by 1 (0 is 'null')
...
struct trace_t {
	...
	gvec3_t			endPos = vec3_origin;		// final position
	cplane_t		plane = {};					// surface normal at impact
	csurface_t		*surface = {};				// surface hit
	contents_t		contents = CONTENTS_NONE;	// contents on other side of surface hit
	gentity_t		*ent = nullptr;				// not set by CM_*() functions
...
enum pmflags_t : uint16_t {
	...
	PMF_TIME_WATERJUMP = bit_v<3>, // pmTime is waterjump
	PMF_TIME_LAND = bit_v<4>,		// pmTime is time before rejump
	PMF_TIME_KNOCKBACK = bit_v<5>, // pmTime is non-moving time
...
struct pmove_state_t {
	pmtype_t			pmType = PM_NORMAL;

	Vector3				origin = vec3_origin;
	Vector3				velocity = vec3_origin;
	pmflags_t			pmFlags = PMF_NONE; // ducked, jump_held, etc
	uint16_t			pmTime = 0;
	int16_t				gravity = 800;
...
struct PMove {
	...
	gvec3_t			viewAngles = vec3_origin; // clamped

	gvec3_t			mins = vec3_origin, maxs = vec3_origin; // bounding box size
	...
	gentity_t		*player = nullptr; // opaque handle
	...
	// [KEX] results (out)
	gvec4_t			screenBlend = { 0, 0, 0, 0 };
	refdef_flags_t	rdFlags = RDF_NONE; // merged with rdFlags from server
	bool			jumpSound = false; // play jump sound
	bool			stepClip = false; // we clipped on top of an object from below
	float			impactDelta = 0.0f; // impact delta, for falling damage
};
...
//
enum MonsterMuzzleFlashID : uint16_t {
	MZ2_UNUSED_0,
...
union player_skinnum_t {
	int32_t         skinNum = 0;
	struct {
		uint8_t     clientNum; // client index
		uint8_t     viewWeaponIndex; // vwep index
		int8_t      viewHeight; // viewHeight
		uint8_t     teamIndex : 4; // team #; note that teams are 1-indexed here, with 0 meaning no team
		// (spectators in CTF would be 0, for instance)
		uint8_t     poiIcon : 4;   // poi icon; 0 default friendly, 1 dead, others unused
	};
};
...
struct entity_state_t {
	uint32_t		number = 0; // entity index

	gvec3_t			origin = vec3_origin;
	gvec3_t			angles = vec3_origin;
	gvec3_t			oldOrigin = vec3_origin; // for lerping
	int32_t			modelIndex = 0;
	int32_t			modelIndex2 = 0, modelIndex3 = 0, modelIndex4 = 0; // weapons, CTF flags, etc
	int32_t			frame = 0;
	int32_t			skinNum = 0;
	Effect			effects = EF_NONE; // PGM - we're filling it, so it needs to be unsigned
	renderfx_t		renderFX = RF_NONE;
	uint32_t		solid = 0;   // for client side prediction
	int32_t			sound = 0;   // for looping sounds, to guarantee shutoff
	entity_event_t	event;   // impulse events -- muzzle flashes, footsteps, etc
	// events only go out for a single frame, they
	// are automatically cleared each frame
	float			alpha = 0.0f;   // [Paril-KEX] alpha scalar; 0 is a "default" value, which will respect other
	// settings (default 1.0 for most things, EF_TRANSLUCENT will default this
	// to 0.3, etc)
	float			scale = 1.0f;   // [Paril-KEX] model scale scalar; 0 is a "default" value, like with alpha.
	uint8_t			instanceBits = 0; // [Paril-KEX] players that *can't* see this entity will have a bit of 1. handled by
...
struct player_state_t {
	...
	gvec3_t			viewAngles = vec3_origin;  // for fixed views
	gvec3_t			viewOffset = vec3_origin;  // add to pmovestate->origin
	gvec3_t			kickAngles = vec3_origin; // add to view direction to get render angles

	// set by weapon kicks, pain effects, etc

	gvec3_t			gunAngles = vec3_origin;
	gvec3_t			gunOffset = vec3_origin;
	int32_t			gunIndex = 0;
	int32_t			gunSkin = 0; // [Paril-KEX] gun skin #
	int32_t			gunFrame = 0;
	int32_t			gunRate = 0; // [Paril-KEX] tickrate of gun animations; 0 and 10 are equivalent

	std::array<float, 4> screenBlend = {}; // rgba full screen effect
	std::array<float, 4> damageBlend = {}; // [Paril-KEX] rgba damage blend effect

	float			fov = 90.0f; // horizontal field of view

	refdef_flags_t	rdFlags = RDF_NONE; // refdef flags

	std::array<int16_t, MAX_STATS> stats = {}; // fast status bar updates

	uint8_t			teamID = 0; // team identifier
};
...
struct svc_fog_data_t {
	...
	bits_t      bits		= {};
	float       density		= 0.0f; // bits & BIT_DENSITY
	uint8_t     skyfactor	= 0x00; // bits & BIT_DENSITY
	uint8_t     red			= 0x00; // bits & BIT_R
	uint8_t     green		= 0x00; // bits & BIT_G
	uint8_t     blue		= 0x00; // bits & BIT_B
	uint16_t    time		= 0x00; // bits & BIT_TIME
...
struct sv_entity_t {
	...
	armorInfo_t		armorInfo[MAX_ARMOR_TYPES];
};
...
enum class BoxEntitiesResult_t {
...
struct game_import_t {
	...
	void		(*positionedSound)(gvec3_cref_t origin, gentity_t *ent, soundchan_t channel, int soundIndex, float volume, float attenuation, float timeofs);
	...
	void		(*localSound)(gentity_t *target, gvec3_cptr_t origin, gentity_t *ent, soundchan_t channel, int soundIndex, float volume, float attenuation, float timeofs, uint32_t dupe_key);
	...
	cvar_t		*(*cvarSet)(const char *var_name, const char *value);
	cvar_t		*(*cvarForceSet)(const char *var_name, const char *value);
...
struct game_export_t {
	...
	// The entity array is allocated in the game dll so it
	// can vary in size from one game to another.
	//
	// The size will be fixed when ge->Init() is called
	gentity_t	*gentities;
	size_t      gentitySize;
	uint32_t    numEntities; // current number, <= maxEntities
...
struct cgame_import_t {
	...
	uint64_t	(*CL_ClientRealTimeUnscaled) ();
...
struct cgame_export_t {
	...
	// [Paril-KEX]
	void			(*GetMonsterFlashOffset)(MonsterMuzzleFlashID id, gvec3_ref_t offset);
	...
	void			(*Wheel_WeapNext)();
	void			(*Wheel_WeapPrev)();
	void			(*Wheel_ApplyButtons)(button_t *cmdButtons);
	void			(*Wheel_ClearInput)();
	bool			(*Wheel_IsOpen)();
	float			(*Wheel_TimeScale)();
	bool			(*Wheel_AllowAttack)();
