...
static const vid_driver_t ui_vid = {
    .set_clipboard_data = UI_SetClipboardData,
};

const vid_driver_t *vid = &ui_vid;

void CG_UI_SetImport(const cgame_ui_import_t *import)
{
    uii = import;
...
{
    return uii ? uii->Cmd_ArgsFrom(arg) : "";
}

char *Cmd_RawArgsFrom(int arg)
{
    return uii ? uii->Cmd_RawArgsFrom(arg) : "";
}
