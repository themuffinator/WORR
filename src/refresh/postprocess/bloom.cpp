...
namespace {
	...
	class BloomStateGuard {
	public:
		BloomStateGuard() noexcept
		{
			previousFbo_ = 0;
			viewport_[0] = viewport_[1] = viewport_[2] = viewport_[3] = 0;
			qglGetIntegerv(GL_FRAMEBUFFER_BINDING, &previousFbo_);
			qglGetIntegerv(GL_VIEWPORT, viewport_);
			previousTexture_ = gls.texnums[TMU_TEXTURE];
			previousLightmap_ = gls.texnums[TMU_LIGHTMAP];
			previousGlowmap_ = gls.texnums[TMU_GLOWMAP];
		}

		~BloomStateGuard()
		{
			if (previousFbo_ >= 0)
				qglBindFramebuffer(GL_FRAMEBUFFER, static_cast<GLuint>(previousFbo_));
			qglViewport(viewport_[0], viewport_[1], viewport_[2], viewport_[3]);
			GL_ForceTexture(TMU_TEXTURE, previousTexture_);
			GL_ForceTexture(TMU_LIGHTMAP, previousLightmap_);
			GL_ForceTexture(TMU_GLOWMAP, previousGlowmap_);
		}

	private:
		GLint previousFbo_;
		GLint viewport_[4];
		GLuint previousTexture_;
		GLuint previousLightmap_;
		GLuint previousGlowmap_;
	};
	...
	Composites the bloom texture onto the scene or displays the debug bloom texture.
	=============
	*/
...
Executes the bloom downsample, filtering, and composite passes while preserving caller state.
=============
*/
void BloomEffect::render(const BloomRenderContext& ctx)
{
	...
	if (useBlur && !bloomError) {
		const float texelScale = blurScale;
		for (int pass = 0; pass < passes; ++pass) {
			for (int axis = 0; axis < 2 && !bloomError; ++axis) {
	...
	const GLuint bloomTexture = bloomError ? 0 : currentTexture;

	runDepthOfField();

	const GLuint colorTexture = (canRunDepthOfField && ctx.dofTexture) ? ctx.dofTexture : ctx.sceneTexture;
	CompositeBloom(ctx, colorTexture, bloomTexture, !bloomError);
}
