...
static void MVD_GameInit(void)
{
    ...
    mvd_clients = MVD_Mallocz(sizeof(mvd_clients[0]) * svs.maxclients);
    edicts = MVD_Mallocz(sizeof(edicts[0]) * (svs.maxclients + 1));
    ...
    mvd_ge.num_edicts = svs.maxclients + 1;
    mvd_ge.max_edicts = svs.maxclients + 1;
