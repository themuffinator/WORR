...
static void Con_Dump_f(void) {
  ...
  if (FS_CloseFile(f))
    Com_EPrintf("$e_auto_3240af950c4e", name);
  else
    Com_Printf("$e_auto_72168c60abd6", name);
}
...
static void start_message_mode(chatMode_t mode) {
  if (cls.state != ca_active || cls.demo.playback) {
    Com_Printf("$e_auto_5d5e456dd309");
    return;
  }
...
static void Con_RemoteMode_f(void) {
  ...
  if (Cmd_Argc() != 3) {
    Com_Printf("$e_auto_8b3a3eed2c7e", Cmd_Argv(0));
    return;
  }
...
static void con_timestampscolor_changed(cvar_t *self) {
  if (!SCR_ParseColor(self->string, &con.ts_color)) {
    Com_WPrintf("$e_auto_c4616e5ce1be", self->string, self->name);
    Cvar_Reset(self);
