...
static void Keybind_Update(menuFrameWork_t *menu)
{
    ...
    for (i = 0; i < menu->nitems; i++) {
        k = static_cast<menuKeybind_t *>(menu->items[i]);
...
{
    menuKeybind_t *k = static_cast<menuKeybind_t *>(arg);
...
=================
*/
static menuSound_t Field_Key(menuField_t *f, int key)
{
...
=================
*/
static menuSound_t Field_Char(menuField_t *f, int key)
{
    bool ret;
...
=================
*/
static menuSound_t SpinControl_DoEnter(menuSpinControl_t *s)
{
    if (!s->numItems)
        return QMS_BEEP;
...
=================
*/
static menuSound_t SpinControl_DoSlide(menuSpinControl_t *s, int dir)
{
    if (!s->numItems)
        return QMS_BEEP;
...
void Menu_AddItem(menuFrameWork_t *menu, void *item)
{
    ...
    if (!menu->nitems) {
        menu->items = static_cast<void **>(UI_Malloc(MIN_MENU_ITEMS * sizeof(void *)));
    } else {
        menu->items = static_cast<void **>(
            Z_Realloc(menu->items, Q_ALIGN(menu->nitems + 1, MIN_MENU_ITEMS) * sizeof(void *)));
    }
...
void Menu_Init(menuFrameWork_t *menu)
{
    ...
    for (i = 0; i < menu->nitems; i++) {
        item = menu->items[i];
        menuCommon_t *common = static_cast<menuCommon_t *>(item);

        focus |= common->flags & QMF_HASFOCUS;
        switch (common->type) {
        case MTYPE_FIELD:
            Field_Init(static_cast<menuField_t *>(item));
            break;
        case MTYPE_SLIDER:
            Slider_Init(static_cast<menuSlider_t *>(item));
            break;
        case MTYPE_LIST:
            MenuList_Init(static_cast<menuList_t *>(item));
            break;
        ...
        case MTYPE_STRINGS:
        case MTYPE_TOGGLE:
            SpinControl_Init(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_EPISODE:
            EpisodeControl_Init(static_cast<menuEpisodeSelector_t *>(item));
            break;
        case MTYPE_UNIT:
            UnitControl_Init(static_cast<menuUnitSelector_t *>(item));
            break;
        case MTYPE_IMAGESPINCONTROL:
            ImageSpinControl_Init(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_ACTION:
        case MTYPE_SAVEGAME:
        case MTYPE_LOADGAME:
            Action_Init(static_cast<menuAction_t *>(item));
            break;
        case MTYPE_SEPARATOR:
            Separator_Init(static_cast<menuSeparator_t *>(item));
            break;
        case MTYPE_STATIC:
            Static_Init(static_cast<menuStatic_t *>(item));
            break;
        case MTYPE_KEYBIND:
            Keybind_Init(static_cast<menuKeybind_t *>(item));
            break;
        case MTYPE_BITMAP:
            Bitmap_Init(static_cast<menuBitmap_t *>(item));
            break;
        default:
    ...
    if (!focus && menu->nitems) {
        item = menu->items[0];
        menuCommon_t *common = static_cast<menuCommon_t *>(item);
        common->flags |= QMF_HASFOCUS;
        if (common->status) {
            menu->status = common->status;
        }
    }
    ...
    for (i = 0; i < menu->nitems; i++) {
        item = menu->items[i];
        rc = &static_cast<menuCommon_t *>(item)->rect;
...
//
    for (i = 0; i < menu->nitems; i++) {
        item = menu->items[i];
        menuCommon_t *common = static_cast<menuCommon_t *>(item);
        if (common->flags & QMF_HIDDEN) {
            continue;
        }

        switch (common->type) {
        case MTYPE_FIELD:
            Field_Draw(static_cast<menuField_t *>(item));
            break;
        case MTYPE_SLIDER:
            Slider_Draw(static_cast<menuSlider_t *>(item));
            break;
        case MTYPE_LIST:
            MenuList_Draw(static_cast<menuList_t *>(item));
            break;
        case MTYPE_SPINCONTROL:
        case MTYPE_BITFIELD:
        case MTYPE_PAIRS:
        ...
        case MTYPE_EPISODE:
        case MTYPE_UNIT:
            SpinControl_Draw(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_ACTION:
        case MTYPE_SAVEGAME:
        case MTYPE_LOADGAME:
            Action_Draw(static_cast<menuAction_t *>(item));
            break;
        case MTYPE_SEPARATOR:
            Separator_Draw(static_cast<menuSeparator_t *>(item));
            break;
        case MTYPE_STATIC:
            Static_Draw(static_cast<menuStatic_t *>(item));
            break;
        case MTYPE_KEYBIND:
            Keybind_Draw(static_cast<menuKeybind_t *>(item));
            break;
        case MTYPE_BITMAP:
            Bitmap_Draw(static_cast<menuBitmap_t *>(item));
            break;
        case MTYPE_IMAGESPINCONTROL:
            ImageSpinControl_Draw(static_cast<menuSpinControl_t *>(item));
            break;
        default:
            Q_assert(!"unknown item type");
        }
        ...
        if (ui_debug->integer) {
            UI_DrawRect8(&common->rect, 1, 223);
        }
    }
...
bool Menu_Push(menuFrameWork_t *menu)
{
    ...
    for (i = 0; i < menu->nitems; i++) {
        ...
        switch (common->type) {
        case MTYPE_SLIDER:
            Slider_Push(static_cast<menuSlider_t *>(item));
            break;
        case MTYPE_BITFIELD:
            BitField_Push(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_PAIRS:
            Pairs_Push(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_STRINGS:
            Strings_Push(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_SPINCONTROL:
            SpinControl_Push(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_TOGGLE:
            Toggle_Push(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_KEYBIND:
            Keybind_Push(static_cast<menuKeybind_t *>(item));
            break;
        case MTYPE_FIELD:
            Field_Push(static_cast<menuField_t *>(item));
            break;
        case MTYPE_SAVEGAME:
        case MTYPE_LOADGAME:
            Savegame_Push(static_cast<menuAction_t *>(item));
            break;
        case MTYPE_IMAGESPINCONTROL:
            ImageSpinControl_Push(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_EPISODE:
            Episode_Push(static_cast<menuEpisodeSelector_t *>(item));
            break;
        case MTYPE_UNIT:
            Unit_Push(static_cast<menuUnitSelector_t *>(item));
            break;
        default:
            break;
        }
...
void Menu_Pop(menuFrameWork_t *menu)
{
    ...
    for (i = 0; i < menu->nitems; i++) {
        ...
        switch (common->type) {
        case MTYPE_SLIDER:
            Slider_Pop(static_cast<menuSlider_t *>(item));
            break;
        case MTYPE_BITFIELD:
            BitField_Pop(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_PAIRS:
            Pairs_Pop(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_STRINGS:
            Strings_Pop(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_SPINCONTROL:
            SpinControl_Pop(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_TOGGLE:
            Toggle_Pop(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_KEYBIND:
            Keybind_Pop(static_cast<menuKeybind_t *>(item));
            break;
        case MTYPE_FIELD:
            Field_Pop(static_cast<menuField_t *>(item));
            break;
        case MTYPE_IMAGESPINCONTROL:
            ImageSpinControl_Pop(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_EPISODE:
            Episode_Pop(static_cast<menuEpisodeSelector_t *>(item));
            break;
        case MTYPE_UNIT:
            Unit_Pop(static_cast<menuUnitSelector_t *>(item));
            break;
        default:
            break;
        }
...
void Menu_Free(menuFrameWork_t *menu)
{
    ...
    for (i = 0; i < menu->nitems; i++) {
        ...
        case MTYPE_SAVEGAME:
        case MTYPE_LOADGAME:
            Action_Free(static_cast<menuAction_t *>(item));
            break;
        case MTYPE_SLIDER:
            Slider_Free(static_cast<menuSlider_t *>(item));
            break;
        case MTYPE_BITFIELD:
        case MTYPE_TOGGLE:
            BitField_Free(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_PAIRS:
            Pairs_Free(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_SPINCONTROL:
        case MTYPE_STRINGS:
        case MTYPE_EPISODE:
            SpinControl_Free(static_cast<menuSpinControl_t *>(item));
            break;
        case MTYPE_UNIT:
            Unit_Free(static_cast<menuUnitSelector_t *>(item));
            break;
        case MTYPE_KEYBIND:
            Keybind_Free(static_cast<menuKeybind_t *>(item));
            break;
        case MTYPE_FIELD:
            Field_Free(static_cast<menuField_t *>(item));
            break;
        case MTYPE_SEPARATOR:
            Z_Free(item);
            break;
        case MTYPE_BITMAP:
            Bitmap_Free(static_cast<menuBitmap_t *>(item));
            break;
        case MTYPE_IMAGESPINCONTROL:
            ImageSpinControl_Free(static_cast<menuSpinControl_t *>(item));
            break;
        default:
            break;
        }
