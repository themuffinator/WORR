...
{
    int cmp = memcmp(s1->address.ip.u8, s2->address.ip.u8, sizeof(s1->address.ip.u8));
    if (cmp > 0)
        return 1;
    if (cmp < 0)
        return -1;
...
void M_Menu_Servers(void)
{
    ...
    m_servers.menu.name     = const_cast<char *>("servers");
    m_servers.menu.title    = const_cast<char *>("Server Browser");
