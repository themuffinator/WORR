...
static void WriteCache(void)
{
    ...
    for (i = m_demos.numDirs; i < m_demos.list.numItems; i++) {
        e = static_cast<demoEntry_t *>(m_demos.list.items[i]);
...
static menuSound_t Change(menuCommon_t *self)
{
    ...
    if (!m_demos.list.numItems) {
        m_demos.menu.status = const_cast<char *>("No demos found");
        return QMS_BEEP;
    }
...
static void BuildList(void)
{
    ...
    m_demos.menu.status = const_cast<char *>("Building list...");
    SCR_UpdateScreen();
    ...
    // alloc entries
    m_demos.list.items = static_cast<void **>(
        UI_Malloc(sizeof(demoEntry_t *) * (numDirs + numDemos + 1)));
...
static menuSound_t LeaveDirectory(void)
{
    ...
    for (i = 0; i < m_demos.numDirs; i++) {
        demoEntry_t *e = static_cast<demoEntry_t *>(m_demos.list.items[i]);
...
void M_Menu_Demos(void)
{
    ...
    m_demos.menu.name = const_cast<char *>("demos");
    m_demos.menu.title = const_cast<char *>("Demo Browser");
