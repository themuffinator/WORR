...
static void CL_Record_f(void)
{
    ...
    for (i = 1; i < cl.csr.max_edicts; i++) {
        ...
        Q2PROTO_MakeEntityDelta(&cls.demo.q2proto_context, &baseline->delta_state, NULL, &packed_entity,
...
static void CL_PlayDemo_f(void)
{
    ...
    if (!cls.demo.z_buffer) {
        ...
        cls.demo.z_buffer = static_cast<byte *>(Z_Malloc(cls.demo.z_buffer_size));
    }
...
void CL_EmitDemoSnapshot(void)
{
    ...
    if (cls.demo.buffer.overflowed) {
        ...
        snap = static_cast<demosnap_t *>(Z_Malloc(sizeof(*snap) + cls.demo.buffer.cursize - 1));
        ...
        cls.demo.snapshots = static_cast<demosnap_t **>(Z_Realloc(
            cls.demo.snapshots,
            sizeof(cls.demo.snapshots[0]) * Q_ALIGN(cls.demo.numsnapshots + 1, MIN_SNAPSHOTS)));
