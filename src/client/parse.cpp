...
static void CL_ParseServerData(const q2proto_svc_serverdata_t *serverdata)
{
    ...
    if (cls.serverProtocol == PROTOCOL_VERSION_R1Q2) {
        ...
        cl.esFlags = static_cast<msgEsFlags_t>(cl.esFlags | MSG_ES_BEAMORIGIN);
        if (cls.q2proto_ctx.features.has_solid32) {
            cl.esFlags = static_cast<msgEsFlags_t>(cl.esFlags | MSG_ES_LONGSOLID);
        }
        cl.pmp.speedmult = 2;
        ...
        if (serverdata->q2pro.extensions_v2) {
            ...
            cl.esFlags = static_cast<msgEsFlags_t>(cl.esFlags | MSG_ES_EXTENSIONS_2);
            cl.psFlags = static_cast<msgPsFlags_t>(cl.psFlags | MSG_PS_EXTENSIONS_2);
            if (cls.protocolVersion >= PROTOCOL_VERSION_Q2PRO_PLAYERFOG)
                cl.psFlags = static_cast<msgPsFlags_t>(cl.psFlags | MSG_PS_MOREBITS);
            PmoveEnableExt(&cl.pmp);
        }
        cl.esFlags = static_cast<msgEsFlags_t>(cl.esFlags | MSG_ES_UMASK | MSG_ES_LONGSOLID);
        if (cls.protocolVersion >= PROTOCOL_VERSION_Q2PRO_BEAM_ORIGIN) {
            cl.esFlags = static_cast<msgEsFlags_t>(cl.esFlags | MSG_ES_BEAMORIGIN);
        }
        if (cls.protocolVersion >= PROTOCOL_VERSION_Q2PRO_SHORT_ANGLES) {
            cl.esFlags = static_cast<msgEsFlags_t>(cl.esFlags | MSG_ES_SHORTANGLES);
        }
        cl.pmp.speedmult = 2;
        ...
        if (cl.game_api == Q2PROTO_GAME_RERELEASE)
            ...
            cl.csr = cs_remap_q2pro_new;
        cl.psFlags = static_cast<msgPsFlags_t>(cl.psFlags | MSG_PS_RERELEASE | MSG_PS_EXTENSIONS);
        if (cl.game_api == Q2PROTO_GAME_Q2PRO_EXTENDED_V2)
            cl.psFlags = static_cast<msgPsFlags_t>(cl.psFlags | MSG_PS_EXTENSIONS_2);
        cl.esFlags = static_cast<msgEsFlags_t>(cl.esFlags | MSG_ES_RERELEASE | CL_ES_EXTENDED_MASK);
    ...
    if (cl.csr.extended) {
        cl.esFlags = static_cast<msgEsFlags_t>(cl.esFlags | CL_ES_EXTENDED_MASK);
        cl.psFlags = static_cast<msgPsFlags_t>(cl.psFlags | MSG_PS_EXTENSIONS);
    ...
    if (cls.serverProtocol == PROTOCOL_VERSION_RERELEASE) {
        ...
        cls.demo.esFlags = cl.csr.extended ? static_cast<msgEsFlags_t>(CL_ES_EXTENDED_MASK_2)
                                           : static_cast<msgEsFlags_t>(0);
        cls.demo.psFlags = cl.csr.extended ? static_cast<msgPsFlags_t>(CL_PS_EXTENDED_MASK_2)
...
static bool CL_IsMatchAnnouncement(int level)
{
    int base_level = level & 7;
