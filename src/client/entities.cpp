...
// Q3-style item bobbing
// This causes items to float up and down using a smooth desynced cosine wave
// based on their entity index
...
// KEX
        ...
        if (effects & EF_BOB) {
            ...
            case 2: // Q3A cosine
            {
                ...
                float modulus = (2.0f * M_PI * 20228.0f) / scale;
                float t = (float)((cl.time + 1000) % (int)modulus);
                float bob = 4.0f + cosf(t * scale) * 4.0f;
        ...
        if (effects & EF_ROTATE) {  // some bonus items auto-rotate
            ...
            switch (cl_item_rotate_style->integer) {
            case 0: // no rotation
                autorotate = 0.0f;
                break;
            default:
            case 1: // q2re style
                ...
                break;
            case 2: // q3a style
                autorotate = anglemod((cl.time & 2047) * 360 / 2048.0);
                break;
            }
...
void CL_CalcViewValues(void)
{
    ...
    // **BOBSKIP MODIFICATION**
    // If info_bobskip is enabled, we clear the viewoffset vector to remove all bob and kick.
    // Otherwise, we calculate it normally through interpolation.
    ...
    // This applies the viewoffset, which is now either the real bob/kick or a zero vector.
    VectorAdd(cl.refdef.vieworg, viewoffset, cl.refdef.vieworg);

    // This essential viewheight logic is now always executed correctly.
