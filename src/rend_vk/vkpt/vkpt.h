...
void create_view_matrix(mat4_t matrix, const refdef_t *fd);
...
#define VKPT_QUEUE_WAIT_IDLE(queue) vkpt_queue_wait_idle(queue, __func__)
...
void vkpt_debugdraw_addtext(const vec3_t origin, const vec3_t angles, const char *text, float size, color_t color, uint32_t time, bool depth_test);
