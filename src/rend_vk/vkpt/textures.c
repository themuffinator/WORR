...
load_blue_noise(void)
{
	...
	BufferResource_t buf_img_upload;
	VkResult result = buffer_create(&buf_img_upload, total_size * NUM_BLUE_NOISE_TEX,
		VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	if (result != VK_SUCCESS) {
		Com_EPrintf("blue noise: buffer_create failed (%d)\n", (int)result);
		return result;
	}

	uint16_t *bn_tex = (uint16_t *) buffer_map(&buf_img_upload);
	if (!bn_tex) {
		Com_EPrintf("blue noise: buffer_map failed\n");
		buffer_destroy(&buf_img_upload);
	...
	for(int i = 0; i < num_images; i++) {
		...
		if(!data) {
			if (!fallback_used) {
				Com_WPrintf("blue noise textures missing, using procedural fallback\n");
			}
			fallback_used = true;

			for (int k = 0; k < 4; k++) {
				uint32_t state = 0x9e3779b9u ^ (uint32_t)((i * 4 + k) + 1) * 0x85ebca6bu;
				for (int j = 0; j < (int)img_size; j++) {
					state ^= state << 13;
					state ^= state >> 17;
					state ^= state << 5;
					bn_tex[(i * 4 + k) * img_size + j] = (uint16_t)(state >> 16);
				}
			}
			continue;
		}
