...
static inline const mvertex_t *
bsp_surfedge_vertex(const bsp_t *bsp, const msurfedge_t *surfedge)
{
	const medge_t *edge = bsp->edges + surfedge->edge;
	return bsp->vertices + edge->v[surfedge->vert];
}
	...
	VboPrimitive* primitives_out)
{
	...
	for (int i = 0; i < surf->numsurfedges; i++) {
		const msurfedge_t *src_surfedge = surf->firstsurfedge + i;
		const mvertex_t *src_vert = bsp_surfedge_vertex(bsp, src_surfedge);
...
static bool
get_surf_plane_equation(const bsp_t *bsp, mface_t* surf, float* plane)
{
	...
	{
		const msurfedge_t *se0 = surf->firstsurfedge + (i + 0);
		const msurfedge_t *se1 = surf->firstsurfedge + (i + 1);
		const msurfedge_t *se2 = surf->firstsurfedge + ((i + 2) % surf->numsurfedges);
		const float *v0 = bsp_surfedge_vertex(bsp, se0)->point;
		const float *v1 = bsp_surfedge_vertex(bsp, se1)->point;
		const float *v2 = bsp_surfedge_vertex(bsp, se2)->point;
		vec3_t e0, e1;
...
static bool
is_sky_or_lava_cluster(bsp_mesh_t* wm, const bsp_t *bsp, mface_t* surf, int cluster, int material_id)
{
	if (cluster < 0)
		return false;
	...
	{
		vec4_t plane;
		if (get_surf_plane_equation(bsp, surf, plane))
		{
			if (plane[2] < 0.f)
				return true;
		}
...
collect_surfaces(uint32_t *prim_ctr, bsp_mesh_t *wm, bsp_t *bsp, int model_idx, int (*filter)(uint32_t, uint32_t, int))
{
	...
	for (int i = 0; i < num_faces; i++) {
		...
		if (MAT_IsKind(material_id, MATERIAL_KIND_CAMERA) && wm->num_cameras > 0)
		{
			...
			int camera_id = Com_SlowRand() % (wm->num_cameras * 4);
		...
		for (uint32_t k = 0; k < prims_in_surface; ++k) 
		{
			if (model_idx < 0)
			{
				...
				if (cluster >= 0 && (MAT_IsKind(material_id, MATERIAL_KIND_SKY) || MAT_IsKind(material_id, MATERIAL_KIND_LAVA)))
				{
					...
					if (is_sky_or_lava_cluster(wm, bsp, surf, cluster, material_id) || (cvar_pt_bsp_sky_lights->integer && is_bsp_sky_light))
					{
...
append_light_poly(int* num_lights, int* allocated, light_poly_t** lights)
{
	...
	{
		int new_allocated = max(*allocated * 2, 128);
		light_poly_t *new_lights = Z_Malloc(new_allocated * sizeof(*new_lights));
		if (*lights)
		{
			memcpy(new_lights, *lights, (*allocated) * sizeof(*new_lights));
			Z_Free(*lights);
		}
		*lights = new_lights;
...
collect_sky_and_lava_light_polys(bsp_mesh_t *wm, bsp_t* bsp)
{
	for (int i = 0; i < bsp->numfaces; i++)
	{
		...
		for (int i = 0; i < num_triangles; i++)
		{
			...
			if (is_sky_or_lava_cluster(wm, bsp, surf, light.cluster, surf->texinfo->material->flags) ||
...
static void tinyobj_fs_reader(void *ctx, const char *filename, int is_mtl, const char *obj_filename,
							  char **buf, size_t *len)
{
	tinyobj_fs_reader_t *reader = ctx;
	...
	reader->mtl_buf = file_buffer;
	reader->mtl_len = (size_t)file_size;
	*buf = reader->mtl_buf;
	*len = reader->mtl_len;
}
...
bsp_mesh_load_custom_sky(const char* map_name)
{
	...
	tinyobj_fs_reader_t reader = { 0 };
	reader.obj_buf = file_buffer;
	reader.obj_len = (size_t)file_size;
	...
	int ret = tinyobj_parse_obj(&custom_sky_attrib, &shapes, &num_shapes, &materials,
		&num_materials, filename, tinyobj_fs_reader, &reader, flags);
