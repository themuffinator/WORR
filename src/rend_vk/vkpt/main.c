...
// Client cvars used by vkpt renderer logic.
...
static qhandle_t vkpt_hud_font = 0;
...
vkpt_initialize_all(VkptInitFlags_t init_flags)
{
	...
	for(int i = 0; i < LENGTH(vkpt_initialization); i++) {
		...
		VkResult init_result = VK_SUCCESS;
		if (init->initialize) {
			init_result = init->initialize();
			init->is_initialized = (init_result == VK_SUCCESS);
		} else {
			init->is_initialized = 1;
		}

		if (!init->is_initialized) {
			Com_SetLastError(va("Vulkan: vkpt init '%s' failed (%d)", init->name, (int)init_result));
			Com_EPrintf("vkpt_initialize_all: '%s' failed (%d)\n", init->name, (int)init_result);
			return (init_result != VK_SUCCESS) ? init_result : VK_ERROR_INITIALIZATION_FAILED;
		}
	}
	...
	if ((VKPT_INIT_DEFAULT & init_flags) == init_flags)
	{
		if (!initialize_transparency()) {
			Com_SetLastError("Vulkan: vkpt init 'transparency' failed");
			Com_EPrintf("vkpt_initialize_all: 'transparency' failed\n");
...
vkpt_reload_shader(void)
{
	...
	if (vkpt_load_shader_modules() != VK_SUCCESS) {
		Com_WPrintf("reload_shader: failed to load shader modules\n");
		return;
	}
...
static const char *vk_surface_extension_for_platform(vid_native_platform_t platform)
{
	switch (platform) {
#if defined(_WIN32)
...
static bool vk_query_native_window(vid_native_window_t *out)
{
	if (!out) {
...
{
	const vid_native_window_t *native = &qvk.native_window;
	...
	case VID_NATIVE_WIN32: {
		...
		return vkCreateWin32SurfaceKHR(qvk.instance, &create_info, NULL, &qvk.surface) == VK_SUCCESS;
	}
#endif
#if USE_X11
	case VID_NATIVE_X11: {
		...
		return vkCreateXlibSurfaceKHR(qvk.instance, &create_info, NULL, &qvk.surface) == VK_SUCCESS;
	}
#endif
#if USE_WAYLAND
	case VID_NATIVE_WAYLAND: {
		...
		return vkCreateWaylandSurfaceKHR(qvk.instance, &create_info, NULL, &qvk.surface) == VK_SUCCESS;
	}
#endif
	default:
...
init_vulkan(void)
{
	...
	/* instance extensions */
	const char *surface_extension = vk_surface_extension_for_platform(qvk.native_window.platform);
	if (!surface_extension) {
		Com_SetLastError("Vulkan: unsupported window platform for surface creation");
		return false;
	}
	...
	Com_Printf("Vulkan instance extensions required by platform: \n");
	for (int i = 0; i < LENGTH(base_exts); i++) {
		Com_Printf("  %s\n", base_exts[i]);
	}

	int num_inst_ext_max = LENGTH(base_exts) + LENGTH(vk_requested_instance_extensions) + NUM_OPTIONAL_INSTANCE_EXTENSIONS;
	const char **ext = alloca(sizeof(const char *) * num_inst_ext_max);
	int num_inst_ext_combined = 0;
	memcpy(ext + num_inst_ext_combined, base_exts, sizeof(base_exts));
	num_inst_ext_combined += LENGTH(base_exts);
...
#undef VK_EXTENSION_DO
	...
	if (!vk_create_surface()) {
		const char *err = Com_GetLastError();
		Com_EPrintf("%s\n", err ? err : "Vulkan: surface creation failed");
		return false;
	}
...
static bool
vkpt_get_current_basedir(char *out, size_t out_size)
{
#if defined(_WIN32)
	if (!_getcwd(out, (int)out_size))
		return false;
#else
...
static bool
vkpt_load_shader_from_basedir(const char *basedir, const char *path, char **data, size_t *size)
{
...
// Shader load order: current dir basedir -> user files basedir -> installation basedir.
static size_t
vkpt_load_shader_data(const char *path, char **data)
{
	char cwd[MAX_OSPATH];
	size_t size = 0;
	cvar_t *homedir;
	...
	*data = NULL;
	...
	if (vkpt_get_current_basedir(cwd, sizeof(cwd))) {
		if (vkpt_load_shader_from_basedir(cwd, path, data, &size))
			return size;
	}

	homedir = Cvar_FindVar("homedir");
	if (homedir && homedir->string[0]) {
		if (vkpt_load_shader_from_basedir(homedir->string, path, data, &size))
			return size;
	}

	size = FS_LoadFileEx(path, (void **)data, FS_PATH_BASE | FS_DIR_BASE, TAG_FILESYSTEM);
	return size;
}
...
create_shader_module_from_file(const char *name, const char *enum_name, bool is_rt_shader)
{
	...
	size = vkpt_load_shader_data(path, &data);
	if(!data) {
...
prepare_entities(EntityUploadInfo* upload_info)
{
	...
	const bool first_person_model = cl_player_model && cl_player_model->integer == CL_PLAYER_MODEL_FIRST_PERSON;
...
{
	draw_string_aligned(x + 1, y + 1, 1, flags & ~UI_DROPSHADOW, maxlen, s, COLOR_BLACK);
	draw_string_aligned(x, y, 1, flags & ~UI_DROPSHADOW, maxlen, s, COLOR_WHITE);
}
...
evaluate_reference_mode(reference_mode_t* ref_mode)
{
	if (is_accumulation_rendering_active())
	{
		...
		case 1: {
			...
			vkpt_draw_set_hud_alpha(hud_alpha);
			break;
		}
		case 2:
			vkpt_draw_set_hud_alpha(0.f);
			break;
		}
	}
	else
			...
			const vec3_t sky_matrix[3], bool render_world)
{
	...
	qvk.gpu_slice_width_prev = qvk.gpu_slice_width;
		...
		int camera_cluster_contents = viewleaf ? viewleaf->contents[0] : 0;
...
#undef UBO_CVAR_DO
	...
	ubo->first_person_model = cl_player_model && cl_player_model->integer == CL_PLAYER_MODEL_FIRST_PERSON;
...
void
R_RenderFrame(const refdef_t *fd)
{
	if (!qvk.swap_chain)
		return;
	...
	vkpt_freecam_update(fd->frametime);
	...
	if (cvar_tm_blend_enable->integer) {
		vec4_t blend;
		for (int i = 0; i < 4; i++) {
			blend[i] = min(1.0f, fd->screen_blend[i] + fd->damage_blend[i]);
		}
		Vector4Copy(blend, ubo->fs_blend_color);
	} else {
...
void
R_ModeChanged(int width, int height, int flags)
{
	...
	qvk.draw_width = width;
	qvk.draw_height = height;
...
R_Init(bool total)
{
	...
	cvar_ray_tracing_api = Cvar_Get("ray_tracing_api", "auto", CVAR_ARCHIVE);
	cvar_ray_tracing_api->generator = &ray_tracing_api_g;

	// When nonzero, the Vulkan validation layer is requested
	cvar_vk_validation = Cvar_Get("vk_validation", "0", CVAR_ARCHIVE);
...
#undef UBO_CVAR_LIST
	...
	cl_player_model = Cvar_Get("cl_player_model", "2", CVAR_ARCHIVE);
	...
	IMG_GetPalette();
	MOD_Init();
	vkpt_hud_font = R_RegisterFont("conchars");
	...
	if (vkpt_initialize_all(VKPT_INIT_DEFAULT) != VK_SUCCESS
		|| vkpt_initialize_all(VKPT_INIT_RELOAD_SHADER) != VK_SUCCESS
		|| vkpt_initialize_all(VKPT_INIT_SWAPCHAIN_RECREATE) != VK_SUCCESS) {
		Com_EPrintf("Vulkan: vkpt initialization failed: %s\n", Com_GetLastError());
		vkpt_destroy_all(VKPT_INIT_DEFAULT);
		vkpt_destroy_shader_modules();
		destroy_vulkan();
		if (total && vid && vid->shutdown) {
			vid->shutdown();
...
void
R_SetSky(const char *name, float rotate, bool autorotate, const vec3_t axis)
{
	int     i;
...
void vkpt_free_command_buffers(cmd_buf_group_t* group)
{
	...
	vkpt_freep((void**)&group->buffers);

#ifdef USE_DEBUG
	vkpt_freep((void**)&group->buffer_begin_addrs);
#endif
...
static VkQueue vkpt_resolve_queue(VkQueue queue, const char *context)
{
	if (queue != VK_NULL_HANDLE)
		return queue;

	if (qvk.queue_graphics == VK_NULL_HANDLE && qvk.device != VK_NULL_HANDLE && qvk.queue_idx_graphics >= 0)
		vkGetDeviceQueue(qvk.device, qvk.queue_idx_graphics, 0, &qvk.queue_graphics);

	if (qvk.queue_graphics == VK_NULL_HANDLE) {
		Com_EPrintf("%s: Vulkan graphics queue unavailable\n", context ? context : "vkpt");
		Com_SetLastError("Vulkan: graphics queue unavailable");
		return VK_NULL_HANDLE;
	}

	Com_WPrintf("%s: queue handle missing, using graphics queue fallback\n", context ? context : "vkpt");
	return qvk.queue_graphics;
}

void vkpt_queue_wait_idle(VkQueue queue, const char *context)
{
	queue = vkpt_resolve_queue(queue, context);
	if (queue == VK_NULL_HANDLE)
		return;
...
{
	queue = vkpt_resolve_queue(queue, "vkpt_wait_idle");
	if (queue == VK_NULL_HANDLE)
		return;
...
void vkpt_submit_command_buffer(
	...
	VkFence fence)
{
	queue = vkpt_resolve_queue(queue, "vkpt_submit_command_buffer");
	if (queue == VK_NULL_HANDLE)
		return;
	if (cmd_buf == VK_NULL_HANDLE) {
		Com_EPrintf("vkpt_submit_command_buffer: command buffer is null\n");
		Com_SetLastError("Vulkan: command buffer unavailable");
		return;
	}

	VkResult end_result = vkEndCommandBuffer(cmd_buf);
	if (end_result != VK_SUCCESS) {
		Com_EPrintf("vkpt_submit_command_buffer: vkEndCommandBuffer failed (%s)\n",
			qvk_result_to_string(end_result));
		Com_SetLastError(va("Vulkan: vkEndCommandBuffer failed (%s)", qvk_result_to_string(end_result)));
		return;
	}
	...
	if (submit_result != VK_SUCCESS) {
		Com_EPrintf("vkpt_submit_command_buffer: vkQueueSubmit failed (%s)\n",
			qvk_result_to_string(submit_result));
		Com_SetLastError(va("Vulkan: vkQueueSubmit failed (%s)", qvk_result_to_string(submit_result)));
	}
...
{
	return vkpt_debugdraw_supported();
}
...
{
	vkpt_debugdraw_addtext(origin, angles, text, size, color, time, depth_test);
}
