...
About
-----
WORR is an enhanced, multiplayer oriented Quake 2 client, compatible
with existing Quake 2 ports and licensed under GPLv2. This document provides
descriptions of console variables and commands added to or modified by WORR
...
WORR client supports separation of outgoing packet rate, physics frame rate
...
In addition to this, WORR uses the Q2PRO network protocol to pack several input
...
with large number of small packets and cause packet delay or loss. The Q2PRO
protocol is only in use when connected to a server that supports it.
...
cl_gibs::
    Controls rendering of entities with ‘EF_GIB’ flag set. When using WORR
...
cl_flares::
    Controls rendering of entities with ‘RF_FLARE’ flag set. When using WORR
    protocol, disabling this saves some bandwidth since the server stops
    sending these entities at all. Default value is 1 (enabled).

cl_gun::
    Controls rendering of the player's own gun model. When using R1Q2 or WORR
    protocol, disabling this saves some bandwidth since the server stops
...
cl_updaterate::
    Specifies the perferred update rate requested from WORR servers. Only used
...
net_chantype::
    ...
    connecting to servers that support it. Q2PRO netchan supports application-level
...
s_occlusion::
    Enables line-of-sight occlusion filtering for sound effects. Default value
    is 1.
      - 0 - disable occlusion
      - 1 - enable occlusion

s_auto_focus::
    Specifies the minimum focus level main WORR window should have for sound
...
con_history::
    ...
    WORR, to be reloaded on next startup. Maximum number of history lines is 128.
...
cl_font::
    Font used for drawing HUD text. Default value is "fonts/RussoOne-Regular.ttf".
...
cl_chathud::
    Toggles drawing of the client notification area. Default value is 1.
      - 0 - do not draw notifications
      - 1 - draw notifications in normal color
      - 2 - draw chat notifications in alternative color

cl_chathud_lines::
    Deprecated. Notification area uses fixed line counts (4 normally, 8 while
    message mode is active).

cl_chathud_time::
    Deprecated. Notifications expire after 4 seconds, fading out over the last
    200 ms.

cl_chathud_x::
    Deprecated. Notification placement is anchored to the HUD safe zone.

cl_chathud_y::
    Deprecated. Notification placement is anchored to the HUD safe zone.

cl_crosshair_brightness::
    Brightness of the crosshair. 0.0 = darkest, 1.0 = brightest. Default value
    is 1.0.

cl_crosshair_color::
    Crosshair color index (1-26) using the Quake Live palette. Default value
    is 25. See `docs-dev/quake-live-crosshair-cvars.md` for the palette chart.

cl_crosshair_health::
    Colors the crosshair based on the health statistic. This overrides
    `cl_crosshair_color`. Default value is 0 (use static color).

cl_crosshair_hit_color::
    Crosshair hit color index (1-26) used by hit styles 2, 5, and 8. Default
    value is 1.

cl_crosshair_hit_style::
    Controls crosshair hit feedback. Default value is 2.
      - 0 - off
      - 1 - colorize based on damage dealt
      - 2 - colorize to `cl_crosshair_hit_color`
      - 3 - pulse (large pulse)
      - 4 - damage color + pulse (large)
      - 5 - hit color + pulse (large)
      - 6 - pulse (small, matches `cl_crosshair_pulse`)
      - 7 - damage color + pulse (small)
      - 8 - hit color + pulse (small)

cl_crosshair_hit_time::
    Time, in milliseconds, the crosshair hit effect is displayed. Default
    value is 200.

cl_crosshair_pulse::
    Enables crosshair pulsing when items are picked up. Default value is 1.

cl_crosshair_size::
    Size of crosshair, measured in pixels. Also scales hit marker sizing.
    Default value is 32.

NOTE: Crosshair coloring works best with monochrome crosshair images.
...
Hard coded list of the fullscreen video modes is gone from WORR, you can
...
r_fullscreen::
    ...
    ‘r_modelist’. Default value is 0, which means to run in windowed mode.
...
win_flip_on_switch::
    ...
    switching from fullscreen WORR to another application or desktop.  Default
    value is 0 (don't switch video modes).

r_hwgamma::
    Instructs the video driver to use hardware gamma correction for
    implementing ‘r_gamma’.  Default value is 0 (use software gamma).
...
--------------------
/set r_modelist "640x480@75:32 800x600@75:32"
/set r_fullscreen 2
--------------------
====================
...
The following variables are specific to the Windows port of WORR.
...
gl_beamstyle::
    Specifies drawing style of laser beams. Default value is 0.
      - 0 — textured billboard-type beam (WORR style, very fast to draw)
...
only until the next map was loaded. In WORR this trick is no longer needed
...
r_override_textures::
    ...
    with truecolor replacements (in PNG, JPG, TGA or DDS format) by stripping off
...
r_texture_formats::
    ...
    Default value is "png jpg tga dds".
...
.MD2 model overrides
********************
When WORR attempts to load an alias model from disk, it determines actual
...
cl_ignore_stufftext::
    ...
    whitelisted. WORR server will not allow the client in if it can't query
...
cl_rollhack::
    ...
    models did lean in the opposite direction when strafing. New WORR renderer
    doesn't have this bug, but since many players got used to it, WORR is able
...
NOTE: By default, during demo playback, WORR overrides FOV value stored in
...
NOTE: Recommened way to play demos in WORR is the ‘demo’ command, but
...
TIP: With WORR it is possible to record a demo while playing back another one.
...
.Demo packet sizes
******************
When WORR or R1Q2 protocols are in use, demo written to disk is automatically
...
screenshot [format]::
    ...
    specified by ‘r_screenshot_format’ variable. File name is picked up
    automatically from template specified by ‘r_screenshot_template’
    variable.
...
screenshotpng [filename] [compression]::
    ...
    compression level. Otherwise, saves with ‘r_screenshot_compression’ level.
...
screenshotjpg [filename] [quality]::
    ...
    quality level. Otherwise, saves with ‘r_screenshot_quality’ level.
...
TIP: In WORR, you don't have to issue ‘vid_restart’ after changing graphics
...
WORR client tries to be compatible with other Quake 2 ports, including
original Quake 2 release. Compatibility, however, is defined in terms of full
file format and network protocol compatibility. WORR is not meant to be a
direct replacement of your regular Quake 2 client. Some features are
implemented differently in WORR, some may be not implemented at all. You may
need to review your config and adapt it for WORR. This section tries to
...
- WORR has a built-in OpenGL renderer and doesn't support dynamic loading of
  external renderers. Thus, ‘vid_ref’ cvar has been made read-only and exists
  only for compatibility with tools like Q2Admin.

- WORR supports loading system OpenGL library only. Thus, ‘gl_driver’ cvar has
  been made read-only and exists only for compatibility with tools like
  Q2Admin.

- Changes to ‘gl_modulate’ variable in WORR take effect immediately. To set
...
- ‘gl_particle_*’ series of variables are gone, as well as
  ...
  drawn as textured triangles, WORR supports its own ‘gl_partscale’ variable.
...
- WORR works only with virtual paths constrained to the quake file system.
...
- Single player savegame format has been rewritten from scratch for better
  robustness and portability. Only the ‘baseq2’ game library included in WORR
  distribution has been converted to use the new improved savegame format. WORR
