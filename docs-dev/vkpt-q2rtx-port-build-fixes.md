# Q2RTX vkpt port build fixes (WORR)

This document records the build-focused changes made while porting the Q2RTX vkpt Vulkan renderer into WORR. The goal was to resolve compile and link errors against WORR's external renderer API while keeping changes isolated to `src/rend_vk` and related headers.
