...
In [world.c:392-483](../src/rend_gl/world.c#L392-L483), `GL_DrawBspModel` renders BSP brushes using:
...
But the shadowmap vertex shader at [shader.c:601-608](../src/rend_gl/shader.c#L601-L608) expects only:
```glsl
in vec4 a_pos;  // VERT_ATTR_POS only
```

When `GL_Flush3D` is called for BSP faces in shadow pass, it sets `GLA_VERTEX` only at [tess.c:664](../src/rend_gl/tess.c#L664). However, the issue is that BSP model faces are drawn through:
1. `GL_DrawBspModel` → `GL_AddSolidFace` → `GL_DrawSolidFaces` → `GL_DrawFace` → `GL_Flush3D`

In `GL_DrawFace` shadow pass ([tess.c:769-798](../src/rend_gl/tess.c#L769-L798)), indices are computed but the actual vertex data comes from world buffer. The vertex buffer is bound with `VA_3D` format, but the shadowmap shader is NOT designed to read the stride of `VA_3D` data correctly.
...
In [mesh.c:700-719](../src/rend_gl/mesh.c#L700-L719):
```c
if (glr.shadow_pass) {
...
2. **Uniform block corruption**: At [gl.h:845-848](../src/rend_gl/gl.h#L845-L848), the mesh block overlaps with skybox matrices:
   ```c
   union {
       mat4_t          m_sky[2];
       glMeshBlock_t   mesh;
...
3. **CPU-tessellated meshes (non-GPU lerp)**: At [mesh.c:1270-1290](../src/rend_gl/mesh.c#L1270-L1290), if CPU tessellation is used:
   ```c
   } else {
...
**Code path**: `GL_SelectShadowLights` at [main.c:1700-1756](../src/rend_gl/main.c#L1700-L1756)
...
1. **Cache invalidation**: Static light caching may interfere. At [main.c:1865-1868](../src/rend_gl/main.c#L1865-L1868), if `is_static_shadow && cache_enabled && !dirty`, the layer is copied from cache instead of re-rendered. Dynamic lights are NOT `is_static_shadow`, so this should work.

2. **Scene hash invalidation**: The scene hash at [main.c:1796-1804](../src/rend_gl/main.c#L1796-L1804) may cause issues if entities don't change but dynamic lights move.
...
**File**: [mesh.c](../src/rend_gl/mesh.c#L700-L720)
...
**File**: [mesh.c](../src/rend_gl/mesh.c) - In `GL_DrawAliasModel`, after `setup_frame_scale()`:
...
**File**: [tess.c](../src/rend_gl/tess.c#L769-L798) - In `GL_DrawFace` shadow pass:
...
**File**: [main.c](../src/rend_gl/main.c)
...
**File**: [world.c](../src/rend_gl/world.c#L441)
