...
## Overview
This document records the updates applied to `docs-dev/proposals/menu_rework.md` to align the proposal with the current cgame JSON UI system and expand its coverage. The goal is to ensure the proposal targets the actual UI architecture in the repo and enumerates missing widget support and settings coverage in a concrete way.
