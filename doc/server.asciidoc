...
About
-----
WORR is an enhanced, multiplayer oriented Quake 2 server, compatible
...
sv_downloadserver::
    Specifies the initial portion of URL compatible clients will use for HTTP
    downloading. It must begin with a ‘http://’ or ‘https://’ prefix and
    should typically end with a slash. Default value is empty (no download URL).

TIP: It is highly advisable to setup HTTP downloading on any public Quake 2
server. The easiest way to do so is to run
https://github.com/skullernet/pakserve[pakserve] on the same machine as Quake 2
server.
...
sv_allow_map::
    ...
    for dedicated servers, 1 otherwise.
...
sv_max_packet_entities::
    Maximum number of entities in client frame. Default value is 0, which is
    equivalent to 128 for non-extended servers and 512 for extended servers.

sv_trunc_packet_entities::
    When client frame is about to overflow, truncate packetentities instead of
    dropping client frame. This does not break delta compression and should be
    generally safe to use. Some negative effects like entities temporary
    freezing, or delayed spawning can be observed. Only relevant for legacy
    clients not using WORR network channel implementation. Default value is 1
    (enabled).

sv_prioritize_entities::
    Sort entities by priority if number of entities in client frame exceeds
    ‘sv_max_packet_entities’ limit, and throw out low priority entities.
    Default value is 0, which simply throws out entities with higher numbers
    that don't fit into frame. Sorting is potentially CPU intensive and thus
    disabled by default.
...
TIP: WORR clients can stream compressed downloads directly from .pkz archives
...
.Server modes
*************
WORR server can run in either MVD/GTV ‘server’ or ‘client’ mode. ‘Server’ mode
...
errors and jerky movement.  WORR is able to work around this bug and make
player movement speed equal in all directions underwater.
***************
...
sys_console::
    ...
    running a dedicated server and 0 otherwise.
      ...
      - 1 — print to stdout and read commands from stdin, but don't assume it
      is a terminal
...
sys_history::
    ...
    exiting WORR, to be reloaded on next startup. Maximum number of history
...
dumpents [filename]::
    ...
    See also ‘map_override_path’ variable description.
...
pickclient <address:port>::
    ...
    (R1Q2 and WORR clients do), must be in passive connection mode and the
...
introduced in the previous section are used here. This is possible since WORR
...
WORR server tries to be compatible with other Quake 2 ports, including
